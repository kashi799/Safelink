		...
		jsonp(`https://www.blogger.com/feeds/${autoSafeLink.safeLinkConfig.blogId}/posts/default?alt=json-in-script&amp;max-results=15`, function(datajson) {
			...
			for (var i = 0; i < tagLinks.length; i++) {
				...
				if (linkExceptionStatus == false || onlyThisLinkStatus == true) {	
					...
					tagLinks[i].href = `${autoSafeLink.safeLinkConfig.siteUrl}${autoSafeLink.safeLinkConfig.safePage}#?${autoSafeLink.safeLinkConfig.parameterName}=${aesCrypto.encrypt(trimString(tagLinks[i].href), trimString(autoSafeLink.safeLinkConfig.secretKey))}`
